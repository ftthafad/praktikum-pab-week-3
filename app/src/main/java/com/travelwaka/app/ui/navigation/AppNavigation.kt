    ...
    NavHost(
        ...
        composable(Routes.LOGIN) {
            LoginScreen(
                onLoginSuccess = { role ->
                    when (role) {
                        "super_admin" -> navController.navigate(Routes.DASHBOARD_APPROVAL) { popUpTo(Routes.LOGIN) { inclusive = true } }
                        else -> navController.navigate(Routes.HOME) { popUpTo(Routes.LOGIN) { inclusive = true } }
                    }
                },
                onNavigateToRegister = { navController.navigate(Routes.REGISTER) },
                onSkip = { navController.navigate(Routes.HOME) { popUpTo(Routes.LOGIN) { inclusive = true } } }
            )
        }
